...
def plot_winsorization_boxplots(
    X_before: pd.DataFrame, X_after: pd.DataFrame, n_cols: int = 4, save_path: str | None = None,
):
    """
    Draws a before/after box plot for every descriptor in X_before (one subplot
    per descriptor, "before" and "after" side by side), so the effect of
    winsorization on outliers/spread is visible column-by-column.

    Only called when winsorization is actually applied (see run_full_workflow).
    """
    import matplotlib.pyplot as plt

    cols = [c for c in X_before.columns if not pd.api.types.is_bool_dtype(X_before[c])]
    n = len(cols)
    ...
    for i, col in enumerate(cols):
        ax = axes_flat[i]
        data = [X_before[col].dropna().values, X_after[col].dropna().values]
        ax.boxplot(data, labels=["before", "after"], showfliers=True)
    ...
    fig.suptitle("Descriptor distributions before vs. after winsorization", fontsize=12)
    fig.tight_layout(rect=[0, 0, 1, 0.97])
    if save_path:
        fig.savefig(save_path, dpi=150)
        print(f"Winsorization box plots saved to {save_path}")
    return fig
...
# =========================================================================== #
# THRESHOLD-SELECTION DIAGNOSTIC PLOTS (5-fold CV set)
# Deliberately kept OUTSIDE run_cv_imputation_model_comparison / run_full_workflow
# so plot styling/layout can be edited independently without touching the
# training logic. Call these AFTER run_full_workflow, using
# results["oof_proba_dict"] and results["y_train"].
# =========================================================================== #

def plot_roc_curve(y_true, proba, save_path: str | None = None):
    """
    ROC curve: TPR (sensitivity, BBB+) vs. FPR (1 - specificity) across all
    thresholds, on the CV out-of-fold probabilities. Annotates AUC-ROC.
    """
    import matplotlib.pyplot as plt
    ...
    ax.plot(fpr, tpr, color="tab:blue", label=f"ROC curve (AUC = {auc:.3f})")
    ax.plot([0, 1], [0, 1], "k--", alpha=0.5, label="Chance")
    ax.set_xlabel("False Positive Rate (1 - specificity)")
    ax.set_ylabel("True Positive Rate (sensitivity)")
    ax.set_title("ROC curve (5-fold CV, out-of-fold)")
    ax.legend()
    ax.set_xlim(-0.02, 1.02)
    ax.set_ylim(-0.02, 1.02)
    fig.tight_layout()
    if save_path:
        resolved_path = _resolve_save_path(save_path, "roc_curve.png")
        fig.savefig(resolved_path, dpi=150)
        print(f"ROC curve saved to {resolved_path}")
    return fig


def plot_mcc_vs_threshold(
    y_true, proba, threshold: float | None = None, save_path: str | None = None,
):
    """
    MCC as a function of decision threshold, on the same probabilities the
    threshold was actually chosen from -- visualizes WHY the selected
    threshold is where it is (the peak, or the constrained peak if
    TNR>=TPR ruled out the unconstrained maximum elsewhere).
    """
    import matplotlib.pyplot as plt
    ...
    mcc_vals = [matthews_corrcoef(y_true, (proba >= t).astype(int)) for t in thresholds]

    fig, ax = plt.subplots(figsize=(7, 5))
    ax.plot(thresholds, mcc_vals, color="tab:green")
    if threshold is not None:
        chosen_mcc = matthews_corrcoef(y_true, (proba >= threshold).astype(int))
        ax.axvline(threshold, color="k", linestyle="--", alpha=0.7)
        ax.plot([threshold], [chosen_mcc], "ko", markersize=8, zorder=5,
                label=f"Selected: threshold={threshold:.3f}, MCC={chosen_mcc:.3f}")
        ax.legend()
    ax.set_xlabel("Decision threshold")
    ax.set_ylabel("MCC")
    ax.set_title("MCC across decision thresholds (5-fold CV, out-of-fold)")
    fig.tight_layout()
    if save_path:
        resolved_path = _resolve_save_path(save_path, "mcc_vs_threshold.png")
        fig.savefig(resolved_path, dpi=150)
        print(f"MCC-vs-threshold plot saved to {resolved_path}")
    return fig


def plot_confusion_matrix_at_threshold(
    y_true, proba, threshold: float, class_names: tuple = ("BBB-", "BBB+"), save_path: str | None = None,
):
    """Confusion matrix heatmap at a specific decision threshold."""
    import matplotlib.pyplot as plt
    ...
    fig.tight_layout()
    if save_path:
        resolved_path = _resolve_save_path(save_path, "confusion_matrix.png")
        fig.savefig(resolved_path, dpi=150)
        print(f"Confusion matrix plot saved to {resolved_path}")
    return fig
...
def _draw_calibration_curve(ax, y_true, proba_dict, class_to_plot, strategy, n_bins):
    """Shared drawing logic for one axis -- used by both the single-view and
    combined (positive+negative side-by-side) plotting functions below."""
    ax.plot([0, 1], [0, 1], "k--", label="Perfectly calibrated")
    for name, proba in proba_dict.items():
        proba = np.asarray(proba)
        if class_to_plot == "negative":
            y_plot, proba_plot = 1 - y_true, 1 - proba
        elif class_to_plot == "positive":
            y_plot, proba_plot = y_true, proba
        else:
            raise ValueError("class_to_plot must be 'positive' or 'negative'")

        frac_pos, mean_pred = calibration_curve(y_plot, proba_plot, n_bins=n_bins, strategy=strategy)
        ax.plot(mean_pred, frac_pos, marker="o", label=name)

    label = "P(BBB+)" if class_to_plot == "positive" else "P(BBB-)"
    ax.set_xlabel(f"Mean predicted {label}")
    ax.set_ylabel(f"Observed fraction, {label}")
    ax.set_title(f"{'Positive' if class_to_plot == 'positive' else 'Negative'} class ({label})")
    ax.legend(fontsize=8)


def plot_calibration_curve_comparison(
    y_true, proba_dict: dict, save_path: str | None = None,
    class_to_plot: str = "positive", strategy: str = "quantile", n_bins: int = 10,
):
    """
    Single reliability diagram. proba_dict: {imputer_name: oof_proba_array}
    (probabilities of the POSITIVE class, i.e. BBB+, however the models were
    trained -- flipped to the negative class internally if requested).

    class_to_plot:
      "positive" -- standard reliability diagram for P(BBB+) (sklearn default)
      "negative" -- same information mirrored to read directly as P(BBB-):
                    plots calibration_curve(1 - y_true, 1 - proba). Useful
                    when BBB- is the minority class and the class you're
                    actually optimizing for (TNR >= TPR constraint) -- lets
                    you read predicted-P(BBB-) reliability directly instead
                    of mentally flipping the positive-class curve.

    strategy: "quantile" (default here, NOT sklearn's default) bins by equal
      COUNT rather than equal probability width. With an imbalanced minority
      class, "uniform" bins leave the low-probability region (where minority-
      class predictions live) sparse and noisy -- "quantile" keeps every bin
      populated regardless of how the classes split.
    """
    import matplotlib.pyplot as plt

    y_true = np.asarray(y_true)
    fig, ax = plt.subplots(figsize=(6, 6))
    _draw_calibration_curve(ax, y_true, proba_dict, class_to_plot, strategy, n_bins)
    fig.tight_layout()
    if save_path:
        fig.savefig(save_path, dpi=150)
    return fig
...
def plot_calibration_curve_both(
    y_true, proba_dict: dict, save_path: str | None = None,
    strategy: str = "quantile", n_bins: int = 10,
):
    """
    Renders BOTH the positive-class (P(BBB+)) and negative-class (P(BBB-))
    reliability diagrams side by side in one figure, sharing the same
    proba_dict of {name: oof_proba_array}. Recommended default: since neither
    view is redundant in practice (quantile bins differ for each, and BBB- is
    typically the minority/constrained class here), generate both together
    ...
    _draw_calibration_curve(axes[0], y_true, proba_dict, "positive", strategy, n_bins)
    _draw_calibration_curve(axes[1], y_true, proba_dict, "negative", strategy, n_bins)
    fig.tight_layout()
    if save_path:
        fig.savefig(save_path, dpi=150)
    return fig
...
def run_full_workflow(df: pd.DataFrame, config: dict) -> dict:
    ...
    Order of operations: MW filter -> split -> feature selection (fit on
    train) -> winsorization (bounds fit on train, frozen onto test) ->
    CV imputation/model comparison on the training set.
    """
    ...
    descriptor_cols = [c for c in df.columns if c not in {y_col, smiles_col, config.get("mw_col", "MW")}]
    ...
    print("\n=== Step 4: Winsorization (bounds fit on TRAIN only) ===")
    X_train_before_winsor = X_train.copy()
    X_train, winsor_bounds = winsorize_columns(X_train, apply=config.get("winsorize", True))
    X_test = apply_frozen_winsor_bounds(X_test, winsor_bounds)  # frozen train bounds, not re-fit on test
    if config.get("winsorize", True):
        plot_winsorization_boxplots(
            X_train_before_winsor, X_train, save_path=config.get("winsor_plot_path"),
        )
    ...
    calibration_fig = plot_calibration_curve_both(
        y_train, oof_proba_dict, save_path=config.get("calibration_plot_path"),
    )
